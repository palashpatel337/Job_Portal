...
import { useAuth } from "@/context/Auth";
import axios from "axios";
import { ChevronRight, Briefcase, MapPin, IndianRupee, Trash2, Pencil } from "lucide-react";
import React, { useEffect, useState } from "react";
      ...
      {!loading && jobs.length > 0 && (
        <div className="relative z-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobs.map((j) => (
            <div
              key={j._id}
              className="rounded-2xl border backdrop-blur-xl p-6 shadow-lg hover:scale-[1.02] transition duration-200"
              style={{
                background: "rgba(255,255,255,0.04)",
                borderColor: "rgba(196,181,253,0.15)",
              }}
            >
              ...
              <div className="space-y-2 text-sm text-white/60">
                ...
                <p>
                  <span className="text-white/40">Positions:</span> {j.position}
                </p>
              </div>
