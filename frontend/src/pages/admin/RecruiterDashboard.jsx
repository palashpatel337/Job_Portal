...
function RecruiterDashboard() {
  ...
  const getApplicants = async (id) => {
    try {
      ...
      const url = `${import.meta.env.VITE_API_URL}/api/v1/application/${id}/applicants`;
  ...
  useEffect(() => {
    if (selectedJobId && auth?.token) {
      getApplicants(selectedJobId);
    }
  }, [selectedJobId, auth?.token]);
    ...
    >
      ...
      <div className="flex-1 lg:pl-16 lg:pr-48 lg:py-10 mt-5">
        ...
        >
          ...
          {loadingApplicants && selectedJobId && (
            <p className="text-white/50 text-sm">Loading applicants...</p>
          )}
