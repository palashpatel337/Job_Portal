...
//   return (
//     <span
//       className={`text-[10px] font-semibold tracking-widest uppercase px-3 py-1 rounded-full border ${colors}`}
//     >
//       {type}
//     </span>
//   );
...
//         style={{
//           background:
//             "radial-gradient(ellipse at top left, rgba(124,58,237,0.13), transparent 70%)",
//         }}
//       />
...
//       const matchesSearch =
//         !q ||
//         [job.title, job.description, job.companyId?.name, job.requirements]
//           .join(" ")
//           .toLowerCase()
//           .includes(q);
...
//   const hasFilters =
//     searchQuery || locationQuery || activeCategory.label !== "All";
//   const clearFilters = () => {
//     setSearchQuery("");
//     setLocationQuery("");
...
//         />
//         <GlowOrb
//           className="w-56 h-56 top-1/2 -left-10"
//           style={{
...
//           >
//             Curated opportunities from companies that actually care about their
//             people.
//           </p>
...
//                 <span>
//                   {" "}
//                   for <span className="text-purple-300">"{searchQuery}"</span>
//                 </span>
//               )}
//             </p>
//           </div>
...
//           ) : (
//             filteredJobs.map((j) => (
//               <JobCard key={j._id} job={j} onToggleSave={toggleSaveJob} />
//             ))
//           )}
//         </div>
...
//           <div
//             className="relative z-10 flex gap-10 mt-16 pt-8"
//             style={{ borderTop: "1px solid rgba(196,181,253,0.08)" }}
//           >
//             {[
//               { num: `${jobs.length}+`, label: "Active jobs" },
//               {
//                 num: `${new Set(jobs.map((j) => j.companyId?._id)).size}+`,
//                 label: "Companies",
//               },
...
const CATEGORIES = [
  { label: "All", icon: "⊞", keywords: [] },
  {
    label: "Technology",
    icon: "💻",
    keywords: ["software", "developer", "engineer", "backend", "frontend", "fullstack", "devops", "cloud", "data", "ml", "ai", "machine learning", "python", "java", "react", "node", "kubernetes", "docker"],
  },
  {
    label: "Design",
    icon: "🎨",
    keywords: ["design", "ui", "ux", "figma", "product design", "visual", "graphic", "brand", "creative", "prototype", "wireframe"],
  },
  {
    label: "Marketing",
    icon: "📣",
    keywords: ["marketing", "growth", "seo", "content", "social media", "campaign", "digital marketing", "ads", "copywriting", "branding"],
  },
  {
    label: "Finance",
    icon: "📊",
    keywords: ["finance", "accounting", "analyst", "investment", "banking", "financial", "equity", "audit", "tax", "cfa", "budget"],
  },
  {
    label: "Healthcare",
    icon: "🏥",
    keywords: ["health", "medical", "clinical", "pharma", "doctor", "nurse", "hospital", "patient", "biotech", "diagnostic"],
  },
  {
    label: "Research",
    icon: "🔬",
    keywords: ["research", "scientist", "lab", "analysis", "data science", "phd", "academic", "publication", "experiment"],
  },
];
...
function JobBadge({ type = "" }) {
  const t = type.toLowerCase();
  const colors = t.includes("full") ? "bg-violet-500/20 text-violet-300 border-violet-500/30"
...
function SkeletonCard() {
  return (
    <div className="rounded-2xl p-5 flex flex-col gap-4 animate-pulse" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(196,181,253,0.1)" }}>
      <div className="flex items-start justify-between">
        <div className="w-11 h-11 rounded-xl bg-white/10" />
        <div className="w-7 h-7 rounded-lg bg-white/10" />
      </div>
...
function GlowOrb({ className, style }) {
  return <div className={`absolute rounded-full pointer-events-none blur-3xl opacity-25 ${className}`} style={style} />;
}
...
function JobCard({ job: j, onToggleSave }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        {j?.companyId?.logo ? (
          <img src={j.companyId.logo} alt={j.companyId?.name} className="w-11 h-11 rounded-xl object-cover flex-shrink-0" style={{ border: "1px solid rgba(196,181,253,0.2)" }} />
        ) : (
          <div className="w-11 h-11 rounded-xl flex items-center justify-center text-base font-bold text-white flex-shrink-0" style={{ background: "linear-gradient(135deg, #7C3AED, #9D5CF6)" }}>
            {j?.companyId?.name?.charAt(0)?.toUpperCase() || "?"}
          </div>
        )}
        <button
      ...
      <div className="relative">
        <p className="text-[15px] font-semibold text-white capitalize leading-snug tracking-tight">{j?.title}</p>
        <p className="text-sm mt-1 flex items-center gap-1.5" style={{ color: "rgba(255,255,255,0.4)" }}>
          <span>{j?.companyId?.name}</span>
          <span className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: "rgba(196,181,253,0.35)" }} />
          <span>{j?.location}</span>
        </p>
      </div>
      ...
      <div className="relative flex items-center justify-between">
        {j?.salary ? (
          ...
          <span className="text-xs italic" style={{ color: "rgba(255,255,255,0.25)" }}>Salary not disclosed</span>
        )}
        <NavLink to={`/get/${j?._id}`} onClick={(e) => e.stopPropagation()} className="text-xs font-semibold text-purple-200/70 hover:text-white transition-colors">
          Apply →
        </NavLink>
      </div>
    </div>
...
function Homepage() {
  ...
  // Initial Fetch: Sync Jobs and Saved status
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const savedIds = new Set((savedRes.data?.jobs || []).map(j => j._id));

        // Mark which jobs are already saved
  ...
  const toggleSaveJob = async (jobId, isSaved) => {
    try {
      ...
      console.log(error.response?.data?.message || "Error toggling bookmark");
    }
  };

  // Debouncing
  useEffect(() => {
    const timeout = setTimeout(() => setDebouncedSearchQuery(searchQuery), 500);
    return () => clearTimeout(timeout);
  }, [searchQuery]);

  useEffect(() => {
    const timeout = setTimeout(() => setDebouncedLocationQuery(locationQuery), 500);
    return () => clearTimeout(timeout);
  }, [locationQuery]);
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      const q = debouncedSearchQuery.toLowerCase().trim();
      const matchesSearch = !q || [job.title, job.description, job.companyId?.name, job.requirements].join(" ").toLowerCase().includes(q);
      const loc = debouncedLocationQuery.toLowerCase().trim();
      const matchesLocation = !loc || job.location?.toLowerCase().includes(loc);
      const matchesCat = matchCategory(job, activeCategory);
      return matchesSearch && matchesLocation && matchesCat;
  ...
  const categoryCounts = useMemo(() => {
    const counts = {};
    CATEGORIES.forEach((cat) => {
      counts[cat.label] = cat.label === "All" ? jobs.length : jobs.filter((j) => matchCategory(j, cat)).length;
    });
    return counts;
  }, [jobs]);
  ...
  return (
    <Layout>
      <section className="min-h-screen px-4 sm:px-8 lg:px-16 xl:px-28 py-12 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0D0A1A 0%, #1E1333 40%, #2D1B5E 70%, #3B1F7A 100%)" }}>
        ...
        <div className="relative z-10 mb-10">
          <p className="text-xs font-semibold tracking-[0.25em] uppercase mb-3 text-purple-300/60">Latest Openings</p>
          <div className="flex items-end justify-between flex-wrap gap-4 mb-4">
            <h2 className="text-4xl sm:text-5xl font-bold text-white">
              Find your <span className="italic" style={{ background: "linear-gradient(90deg, #C4B5FD, #9D5CF6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>dream role</span>
              <br /><span className="font-normal text-3xl sm:text-4xl opacity-70">without the noise.</span>
            </h2>
            {!loading && (
              <span className="text-xs font-semibold tracking-widest uppercase px-5 py-2 rounded-full bg-purple-500/10 border border-purple-500/25 text-purple-200">
                {filteredJobs.length} of {jobs.length} positions
              </span>
            )}
          </div>

          <div className="flex flex-col sm:row rounded-2xl overflow-hidden max-w-2xl bg-white/5 border border-white/10 backdrop-blur-md">
            <div className="flex items-center gap-3 flex-1 px-4 py-3.5">
              <Search size={16} className="text-purple-400/60" />
              <input type="text" placeholder="Job title or skills..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-transparent outline-none text-sm text-white w-full" />
            </div>
            <div className="flex items-center gap-3 flex-1 px-4 py-3.5 border-t sm:border-t-0 sm:border-l border-white/10">
              <MapPin size={16} className="text-purple-400/60" />
              <input type="text" placeholder="Location..." value={locationQuery} onChange={(e) => setLocationQuery(e.target.value)} className="bg-transparent outline-none text-sm text-white w-full" />
            </div>
          </div>
        </div>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  key={cat.label}
                  onClick={() => setActiveCategory(cat)}
                  className={`flex-shrink-0 flex flex-col items-center gap-1.5 px-5 py-3.5 rounded-xl transition-all border ${isActive ? "bg-purple-500/20 border-purple-400/40" : "bg-white/5 border-white/10"}`}
                  style={{ minWidth: 110 }}
                >
                  <span className="text-xl">{cat.icon}</span>
                  <span className={`text-[13px] font-medium ${isActive ? "text-white" : "text-white/50"}`}>{cat.label}</span>
                  <span className="text-[10px] opacity-40">{categoryCounts[cat.label]} jobs</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {loading ? (
            ...
            <div className="col-span-full py-20 text-center opacity-50">No jobs found. Try clearing filters.</div>
          ) : (
            filteredJobs.map((j) => (
              <JobCard key={j._id} job={j} onToggleSave={toggleSaveJob} />
            ))
